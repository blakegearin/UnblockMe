...
class Search:
    ...
    def execute(self):
        ...
        return None
    def showPath(self, node, display, fileIDToWrite):
        path = self.buildPath(node)
        #Write to files if this puzzle has not been encountered
        if fileIDToWrite != -1: 
            file = open("games.txt", "a")
            file.write(self.initialState.getfileStr(fileIDToWrite))
            file.close()
            
            file = open("solutions.txt", "a")
            prevState = self.initialState
            file.write("SOLUTION " + str(fileIDToWrite) + "\n")
            file.write(str(len(path) - 1) + "\n")
            
        for current in path:
            if fileIDToWrite != -1:
                file.write(current.state.getStateChange(prevState))
            print(current.state)
